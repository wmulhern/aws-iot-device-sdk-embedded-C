...
void printProducerUsage(void){
	printf("Usage:\n"
		"\tPUBLISH: Publish a message to a topic\n\t\t[P|p] topic/name message\n");
}
...
int main(int argc, char **argv) {
	...
	// emtpy strings of max length PATH_MAX
	...
	mqttInitParams.enableAutoReconnect = false; // We enable this later below - WHY?
	...
	char actn[MAX_STRLEN];
	char tpc[MAX_STRLEN];
	char msg[MAX_STRLEN];
	int topic_len = 0;

	// Listen for input to publish
	while(true){
		if (fgets(line, sizeof(line), stdin)) {
		    ...
		    if (num_args >= 1 && num_args <= 3) {
		    	...
		    	if (strncmp(actn,"p", MAX_STRLEN) == 0 || strncmp(actn,"P", MAX_STRLEN) == 0){
		    		// PUBLISH
		    		if (num_args < 3 || strchr(tpc, '/') == NULL){
		    			IOT_WARN("Publish needs two arguments. One specifying the topic in format [topic]/[name]. The second the message to publish to that topic");
		    		} else {
		    			publish(msg, tpc, &client);
		    		}

		    	} else {
		    		IOT_WARN("Invalid input. Try again.");
		    		printProducerUsage();
		    	}
